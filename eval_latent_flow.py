...
def estimate_ball_center_robust(
    ...
    if use_topk_fallback:
        ...
        if vals.numel() >= min_mass:
            ...
            if debug:
                print(
                    f"[topk-fallback] min={frame.min().item():.4f} "
                    f"max={frame.max().item():.4f} mean={frame.mean().item():.4f} "
                    f"k={k}"
                )
    ...
    if debug:
        print(
            f"[center-fail] min={frame.min().item():.4f} "
            f"max={frame.max().item():.4f} mean={frame.mean().item():.4f}"
        )
...
def extract_positions_from_frames_robust(
    frames,
    invert=False,
    threshold=0.5,
    min_mass=3,
    ...
    positions = []
    for frame in frames:
        pos = estimate_ball_center_robust(
            frame_tensor=frame,
            invert=invert,
            threshold=threshold,
            min_mass=min_mass,
...
@torch.no_grad()
def frame_prediction(model, dataloader, device, fm_steps):
    target_frames = []
    predicted_frames = []
    latent_states = []
    state_preds = []
...
def compute_state_metrics_one_step(state_preds, positions, velocities):
    target_state = torch.cat([positions, velocities], dim=1)
    pred_pos = state_preds[:, :2]
    pred_vel = state_preds[:, 2:]

    pos_err = torch.norm(pred_pos - positions, dim=1)
    vel_err = torch.norm(pred_vel - velocities, dim=1)

    return {
        "position_r2": r2_score(positions.numpy(), pred_pos.numpy()),
        "velocity_r2": r2_score(velocities.numpy(), pred_vel.numpy()),
        "position_aee": pos_err.mean().item(),
        "velocity_aee": vel_err.mean().item(),
    }
...
def physics_from_observed_frames_one_step(
    ...
    ctx_positions = extract_positions_from_frames_robust(
        last_context_frames,
        invert=invert,
        threshold=threshold,
        min_mass=min_mass,
...
def build_frame_loader(sequence_dirs, context, invert, grayscale, num_workers, batch_size=64, shuffle=False):
...
def main(args):
    ...
    results = {
        "metadata": {
            ...
            "fallback_thresholds": list(args.fallback_thresholds),
    ...
    target_frames, predicted_frames, hidden_states, state_preds, positions, velocities, last_context_frames = frame_prediction(
        model, test_loader, device, args.fm_steps
    )
    ...
    _, _, train_hidden_states, train_state_preds, train_positions, train_velocities, _ = frame_prediction(
        model, probe_train_loader, device, args.fm_steps
    )
    ...
    state_head_metrics = compute_state_metrics_one_step(
        state_preds=state_preds,
        positions=positions,
        velocities=velocities,
    )

    print("Latent probe physics:")
    print(f"Position R2: {latent_probe_metrics['position_r2']:.4f}")
    print(f"Velocity R2: {latent_probe_metrics['velocity_r2']:.4f}")
    print(f"Position AEE: {latent_probe_metrics['position_aee']:.4f}")
    print(f"Velocity AEE: {latent_probe_metrics['velocity_aee']:.4f}")

    print("State head physics:")
    print(f"Position R2: {state_head_metrics['position_r2']:.4f}")
    print(f"Velocity R2: {state_head_metrics['velocity_r2']:.4f}")
    print(f"Position AEE: {state_head_metrics['position_aee']:.4f}")
    print(f"Velocity AEE: {state_head_metrics['velocity_aee']:.4f}")
    ...
    state_head_rollout_metrics = compute_state_metrics_rollout(
        state_preds=state_preds,
        positions=positions,
        velocities=velocities,
    )
    ...
    print("State head physics:")
    print(f"Position AEE: {state_head_rollout_metrics['aggregate']['position_aee']:.4f}")
    print(f"Velocity AEE: {state_head_rollout_metrics['aggregate']['velocity_aee']:.4f}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Robust evaluation of improved latent flow video model with observed, latent-probe, and state-head metrics"
    )
    ...
    parser.add_argument("--fm_steps", type=int, default=6)
